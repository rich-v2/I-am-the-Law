# This program is supposed to allow you to look up any paragraph of various German tax codes
...
def get_law(law,section,paragraph):
    ...
    ArticleSoup = bs4.BeautifulSoup(res.text)
    ...
    # If sentence is not specified, give entire paragraph
    else:
        text_box.insert(tk.END, gesetzestext.find_all("p")[int(paragraph)-1].getText() +"\n")

# TODO: Übersicht über Urteile?
def get_verdicts(law,section):
    ...
    verdict_frame = tk.Frame(result_window, bg = "blue")
    ...
    desc_frame = tk.Frame(result_window, bg = "red")
    ...
    interact_frame = tk.Frame(result_window, bg = "yellow")
    ...
    b2 = tk.Button(interact_frame,text = "Zum Urteil!", command=lambda: \
...
let user specify 
    ...
    Abs.
    S.
    Button Urteile einzulesen 

Option den gesamten Paragraphen anzeigen zu lassen
Alternativ bis ins Detail
    Urteile neues Popup-Fenster
"""
...
if __name__== "__main__":
    ...
    print_frame = tk.Frame(root, bg = "blue")
    ...
    control_frame = tk.Frame(root, bg = "red")
    ...
    WIDGETS_R2 = [tk.Button(control_frame, text = "Hol mir den Artikel!", command = lambda: get_law(WIDGETS_R1[1].get(), \
